...
class appdevUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        // WongaBloggerBlogBundle_homepage
        if (rtrim($pathinfo, '/') === '') {
            if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                $allow = array_merge($allow, array('GET', 'HEAD'));
                goto not_WongaBloggerBlogBundle_homepage;
            }

            if (substr($pathinfo, -1) !== '/') {
                return $this->redirect($pathinfo.'/', 'WongaBloggerBlogBundle_homepage');
            }

            return array (  '_controller' => 'Wonga\\Blogger\\BlogBundle\\Controller\\PageController::indexAction',  '_route' => 'WongaBloggerBlogBundle_homepage',);
        }
        not_WongaBloggerBlogBundle_homepage:

        // WongaBloggerBlogBundle_about
        if ($pathinfo === '/about') {
            if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                $allow = array_merge($allow, array('GET', 'HEAD'));
                goto not_WongaBloggerBlogBundle_about;
            }

            return array (  '_controller' => 'Wonga\\Blogger\\BlogBundle\\Controller\\PageController::aboutAction',  '_route' => 'WongaBloggerBlogBundle_about',);
        }
        not_WongaBloggerBlogBundle_about:

        // WongaBloggerBlogBundle_contact
        if ($pathinfo === '/contact') {
            if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                $allow = array_merge($allow, array('GET', 'HEAD'));
                goto not_WongaBloggerBlogBundle_contact;
            }

            return array (  '_controller' => 'Wonga\\Blogger\\BlogBundle\\Controller\\PageController::contactAction',  '_route' => 'WongaBloggerBlogBundle_contact',);
        }
        not_WongaBloggerBlogBundle_contact:
